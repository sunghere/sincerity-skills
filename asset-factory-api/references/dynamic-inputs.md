...
| `run:<run_id>/<output_label>` | 이전 run 의 특정 출력 (chain 표준) |
...
2. **`run_id` 보관**: `gen` 응답의 `run_id` 잡아둬야 다음 단계에서 `run:<run_id>/<output>` 참조. 잃어버리면 `af list <project>` 에서 metadata 로 추적 (느림).
